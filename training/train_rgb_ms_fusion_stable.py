"""
train_rgb_ms_fusion_stable.py
RGB + MS 融合 - 基于57.5%成功配置
关键: 不做激进数据增强, 用简单有效的融合
"""
import os
import sys
import argparse
...
class SimpleFusion(nn.Module):
    ...
    def __init__(self):
        ...
        self.ms_weight = nn.Parameter(torch.tensor(0.5))
    ...
    def forward(self, rgb_feat, ms_feat):
        ...
        rgb_w = rgb_w / total
        ms_w = ms_w / total
        fused = rgb_w * rgb_feat + ms_w * ms_feat
        return fused

class RGBMSFusionNetSimple(nn.Module):
...
def main():
    ...
    parser.add_argument("--save_dir", default="./models_rgb_ms_fusion_stable")
    ...
    train_loader, val_loader = build_dataloaders(
        ...
        test_size=0.2,
        random_state=42,
        augment_train=False,  # 关键: 不做数据增强
        balanced=False,  # 不做过度平衡
        modalities=['rgb', 'ms']
    )
    print(f"✅ 数据加载完成")
    
    print("\n🧠 创建模型...")
    model = RGBMSFusionNetSimple(num_classes=5)
    model.to(device)
