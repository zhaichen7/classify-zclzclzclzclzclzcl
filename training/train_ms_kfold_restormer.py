...
def make_dataset(df, ids, data_root, augment):
    tries = [
        lambda: DroughtDataset(
            df, ids,
            data_root=data_root,
            augment=augment,
...
def main():
    ...
    parser.add_argument("--save_dir", default="./models_ms_kfold_restormer")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels)), labels), start=1):
        ...
        train_ds = make_dataset(df, train_ids, args.data_root, True)
        val_ds = make_dataset(df, val_ids, args.data_root, False)
