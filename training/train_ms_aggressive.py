...
class DeepMSNet(nn.Module):
    ...
    def forward(self, x):
        ...
        aux_logits = self.aux_head(x_feat)
...
def train_one_epoch(model, train_loader, criterion, aux_criterion, optimizer, device, scaler, epoch):
    model.train()
    total_loss = 0.0
    correct = 0
    total = 0
    ...
    for _, _, ms, labels in pbar:
        ...
        with torch.cuda.amp.autocast():
            logits, aux_logits = model(ms)
            loss_main = criterion(logits, labels)
            loss_aux = aux_criterion(aux_logits, labels) * 0.2
...
def main():
    ...
    parser.add_argument("--save_dir", default="./models_ms_aggressive")
    ...
    print("="*80)
    print("🚀 MS单模态 激进优化 - 深网络 + 混合精度 + 辅助任务")
    print("="*80)
    ...
    aux_criterion = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=0.15)
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss, train_acc = train_one_epoch(
            model, train_loader, criterion, aux_criterion, optimizer, device, scaler, epoch
        )
