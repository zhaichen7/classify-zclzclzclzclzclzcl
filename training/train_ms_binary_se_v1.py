...
def random_band_dropout(x, p=0.25, max_drop=2):
    """
    x: [B, C, H, W]
    训练时随机把 1~max_drop 个波段置 0，防止模型过度依赖少数通道
    """
...
def train_one_epoch(model, loader, criterion, optimizer, device,
                    ...
                    grad_clip=1.0, band_dropout_p=0.25, band_dropout_max=2):
    model.train()
    total_loss = 0.0
...
def main():
    parser = argparse.ArgumentParser(description="MS 单模态二分类 + SE attention + band dropout")
    ...
    parser.add_argument("--save_dir", default="/home/zcl/addfuse1/two/models_ms_binary_se_v1_run1")
    ...
    print(f"Label distribution: {dict(Counter(mapped_df['label'].tolist()))}")
    print(f"SE reduction      : {args.se_reduction}")
    print(f"Band dropout p    : {args.band_dropout_p}")
    print(f"Band dropout max  : {args.band_dropout_max}")
    print("=" * 90)
    ...
    for epoch in range(1, args.epochs + 1):
        train_metrics = train_one_epoch(
            ...
            grad_clip=1.0,
            band_dropout_p=args.band_dropout_p,
