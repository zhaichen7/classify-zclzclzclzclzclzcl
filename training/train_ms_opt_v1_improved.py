"""
train_ms_opt_v1_improved.py
完全基于train_ms_optimized_v1，只改超参数
关键改动:
  1. 学习率改为1.2e-4 (在1e-4和1.5e-4之间微调)
  2. Gamma改为2.1 (比2.0更强一点)
  3. Label smoothing改为0.12
  4. train/val比例改为85/15 (给训练集更多数据)
  5. patience改为40 (给更多机会)
"""
import os
import sys
import argparse
...
def main():
    parser = argparse.ArgumentParser(description='MS单模态 优化版本')
    ...
    parser.add_argument('--save_dir', default='./models_ms_opt_v1_improved')
    ...
    print("="*80)
    print("🚀 MS单模态 - 优化版本 V1 Improved")
    print("="*80)
    
    print("\n📊 加载数据 (train/val = 85/15)...")
