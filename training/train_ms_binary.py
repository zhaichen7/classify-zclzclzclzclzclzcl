...
def build_model(device, dropout=0.3):
    ...
    model = MSBinaryClassifier(encoder, dropout=dropout).to(device)
    return model
...
def train_one_epoch(model, loader, criterion, optimizer, device, threshold=0.5, label_smoothing=0.0, grad_clip=1.0):
    ...
    for batch in pbar:
        _, _, ms, labels = unpack_batch(batch)
...
def main():
    parser = argparse.ArgumentParser(description="MS 单模态二分类")
    ...
    parser.add_argument("--save_dir", default="/home/zcl/addfuse1/two/models_ms_binary_run1")
