...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    for rgb, _, ms, labels in tqdm(loader, desc="Train", leave=False):
        rgb, ms, labels = rgb.to(device), ms.to(device), labels.to(device)
        optimizer.zero_grad()
        outputs = model(rgb, torch.zeros_like(rgb).to(device), ms)
...
def evaluate(model, loader, criterion, device):
    model.eval()
    total_loss, correct, total = 0, 0, 0
    for rgb, _, ms, labels in tqdm(loader, desc="Val", leave=False):
        rgb, ms, labels = rgb.to(device), ms.to(device), labels.to(device)
        outputs = model(rgb, torch.zeros_like(rgb).to(device), ms)
        loss = criterion(outputs, labels)
        total_loss += loss.item() * labels.size(0)
...
parser.add_argument("--save_dir", default="./models_rgb_ms")
...
print(f"✅ RGB+MS 模型完成! 最佳准确率: {best_val_acc*100:.2f}%")
